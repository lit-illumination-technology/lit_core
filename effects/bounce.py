...
description = "Two lights move down the string and bounce off the ends while changing color"
...
def start(lights, stop_event, speed = 1, **extras):
    ...
    while not stop_event.is_set():
        lights.off()
        lights.set_pixel_hsv(int(x), (x/float(lights.num_leds))%1, 1, 1)
        lights.set_pixel_hsv(int(lights.num_leds - x), ((lights.num_leds - x)/float(lights.num_leds))%1, 1, 1)
        x += dx
        if x + dx >= lights.num_leds or x + dx < 0:
            dx = -dx
        lights.show()
