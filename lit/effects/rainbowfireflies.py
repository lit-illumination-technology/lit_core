...
def update(lights, step, state):
    ...
    for i in range(lights.size):
        ...
        if sd[0] + sd[1] <= step:
            ...
            state['start_duration_colors'][i] = (step+random.randint(0, int(2*avg_off_time)), MIN_ON_TIME+random.randint(0, (AVG_ON_TIME-MIN_ON_TIME)*2), color) 
        brightness = (step-sd[0])/(sd[1]/2)
        if brightness > 1:
            brightness = 2 - brightness
        brightness = max(brightness, 0)
        lights.set_pixel(i, int(sd[2][0]*brightness), int(sd[2][1]*brightness), int(sd[2][2]*brightness))
